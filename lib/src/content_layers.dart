...
class ContentLayersElement extends RenderObjectElement {
  ...
  void mount(Element? parent, Object? newSlot) {
    FtlLogs.contentLayers.fine("ContentLayersElement - mounting");
  ...
  @override
  void activate() {
    FtlLogs.contentLayers.fine("ContentLayersElement - activating");
    super.activate();
  }

  @override
  void deactivate() {
    FtlLogs.contentLayers.fine("ContentLayersElement - deactivating");
  ...
  @override
  void unmount() {
    FtlLogs.contentLayers.fine("ContentLayersElement - unmounting");
  ...
  void _onBuildScheduled() {
    FtlLogs.contentLayers.finer("ON BUILD SCHEDULED");
    ...
    SchedulerBinding.instance.scheduleFrameCallback((timeStamp) {
      FtlLogs.contentLayers.finer("SCHEDULED FRAME CALLBACK");
      if (!mounted) {
        FtlLogs.contentLayers.finer("We've unmounted since the end of the frame. Fizzling.");
        return;
      }
      ...
      if (isContentDirty && isAnyLayerDirty) {
        FtlLogs.contentLayers.fine("Marking needs build because content and at least one layer are both dirty.");
  ...
  bool _isAnyLayerDirty() {
    FtlLogs.contentLayers.finer("Checking if any layer is dirty");
    bool hasDirtyElements = false;

    FtlLogs.contentLayers.finer("Checking underlays");
    for (final underlay in _underlays) {
      FtlLogs.contentLayers.finer(() => " - Is underlay ($underlay) subtree dirty? ${_isSubtreeDirty(underlay)}");
      hasDirtyElements = hasDirtyElements || _isSubtreeDirty(underlay);
    }

    FtlLogs.contentLayers.finer("Checking overlays");
    for (final overlay in _overlays) {
      FtlLogs.contentLayers.finer(() => " - Is overlay ($overlay) subtree dirty? ${_isSubtreeDirty(overlay)}");
...
// This is intentionally static to prevent closure allocation during
  ...
  static void _isSubtreeDirtyVisitor(Element element) {
    ...
    assert(() {
      if (FtlLogs.contentLayers.isLoggable(Level.FINEST)) {
        FtlLogs.contentLayers.finest("Finding dirty children for: $element");
      }
      return true;
    }());
    if (element.dirty) {
      assert(() {
        if (FtlLogs.contentLayers.isLoggable(Level.FINEST)) {
          FtlLogs.contentLayers.finest("Found a dirty child: $element");
        }
        return true;
      }());
      _isDirty = true;
  ...
  @override
  void markNeedsBuild() {
    FtlLogs.contentLayers.finer("ContentLayersElement - marking needs build");
    super.markNeedsBuild();
  }

  void buildLayers() {
    FtlLogs.contentLayers.finer("ContentLayersElement - (re)building layers");
  ...
  void _temporarilyForgetLayers() {
    FtlLogs.contentLayers.finer("ContentLayersElement - temporarily forgetting layers");
    for (final underlay in _underlays) {
...
class RenderContentLayers extends RenderSliver with RenderSliverHelpers {
  ...
  void attach(PipelineOwner owner) {
    FtlLogs.contentLayers.info("Attaching RenderContentLayers to owner: $owner");
    super.attach(owner);
  ...
  @override
  void detach() {
    FtlLogs.contentLayers.info("detach()'ing RenderContentLayers from pipeline");
  ...
  @override
  void performLayout() {
    FtlLogs.contentLayers.info("Laying out ContentLayers");
    if (_content == null) {
    ...
    FtlLogs.contentLayers.fine("Laying out content - $_content");
    (_content!.parentData! as SliverLogicalParentData).layoutOffset = 0.0;
    _content!.layout(constraints, parentUsesSize: true);
    FtlLogs.contentLayers.fine("Content after layout: $_content");
    ...
    // content changes.
    FtlLogs.contentLayers.fine("Building layers");
    invokeLayoutCallback((constraints) {
      _element!.buildLayers();
    });
    FtlLogs.contentLayers.finer("Done building layers");

    FtlLogs.contentLayers.fine("Laying out layers (${_underlays.length} underlays, ${_overlays.length} overlays)");
    ...
    for (final underlay in _underlays) {
      ...
      FtlLogs.contentLayers.fine("Laying out underlay: $underlay");
      underlay.layout(layerConstraints);
    }
    for (final overlay in _overlays) {
      final childParentData = overlay.parentData! as SliverLogicalParentData;
      childParentData.layoutOffset = -constraints.scrollOffset;
      FtlLogs.contentLayers.fine("Laying out overlay: $overlay");
      overlay.layout(layerConstraints);
    }

    _runningLayout = false;
    FtlLogs.contentLayers.finer("Done laying out layers");
  }
